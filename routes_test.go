...
func Test_getShouldReturn404ForInvalidId(t *testing.T) {
	testRouter := routes()
	resp, _ := getTodo(testRouter, "invalid")
...
func Test_patchShouldReturn404ForInvalidId(t *testing.T) {
	testRouter := routes()
	resp, _ := patchTodo(`{"title": "a new title", "completed": true}`, "invalid", testRouter)

	assert.Equal(t, 404, resp.Code)
}

func Test_shouldReturn400ForInvalidJsonWhenPatching(t *testing.T) {
	testRouter := routes()
	_, createdTodo := createTodo(`{"title": "a todo"}`, testRouter)

	resp, _ := patchTodo(`<I'm not json>`, createdTodo.Id, testRouter)
